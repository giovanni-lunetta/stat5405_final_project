...
```{r}
pairs(cbind(football_data$Yards, football_data$DefendersInTheBox, football_data$YardsFromOwnGoal, football_data$Distance))
```
...
```{r}
reduced_model <- lm(log(Yards) ~ Distance + RB + WR + DefendersInTheBox^2 + Temperature + GameHour + DL + LB + YardsFromOwnGoal^2, data=football_data)
