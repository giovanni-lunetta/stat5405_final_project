...
```{r}
reduced_model <- lm(Yards ~ Distance + RB + WR + DefendersInTheBox + Temperature + GameHour + DL + LB + YardsFromOwnGoal, data=football_data)
...
```{r}
reduced_model <- glm(Yards ~ YardLine + Distance + RB + WR + DefendersInTheBox + BL, data=football_data, family = Gamma(link = "log"))
summary(reduced_model)
```
